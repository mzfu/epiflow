...
#'
#' This function takes a dataset of interest as an input and returns a heat map showing
#' Pearson correlation coefficients and their significance. Build based on ggcorrplot and ggplot2.
#'
#' @param dataset A dataset of interest (should all be numeric variables).
#' @param sig_level Significant level to be marked in the heat map, 0.05 by default.
#' @param lab_size Size for label.
#' @param fontsize Size for font.
#' @param width Output figure width.
#' @param height Output figure height.
#' @param save_name Saved name for your figure.
#' @param output_path File path for output figure.
#' @return A heat map for correlations across all covariates.
#' @examples
#' make_heatmap(example_corr, sig_level = 0.95, lab_size = 3,
#' fontsize = 12, width = 9, height = 9, 'example_output.png', output_path)
#' @export

# Use this function for making the heatmap
make_heatmap <- function(dataset, sig_level = 0.95, lab_size = 3, fontsize = 12, width = 8, height = 8, save_name, output_path) {
  ...
  cor_plot = mycorrplot(cor_data, type = "lower", p.mat = p.mat, lab_size = lab_size, lab = T, significant  = "pch",
                        legend.title = 'Pearson\nCorrelation', outline.color = "white", tl.cex = 11, sig.level = sig_level,
  ...
  ggsave(save_name, plot = cor_plot, path = output_path, width = width, height = height)
